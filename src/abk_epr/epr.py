...
class ExifRename:
    ...
    async def _move_and_rename_files(self, key, value) -> None:
        ...
        if len(rename_files_list) > 0:
            rename_tasks = [self._rename_file_async(old_name, new_name) for old_name, new_name in rename_files_list]  # noqa: E501
        ...
        if key == ListType.RAW_IMAGE_DICT.value:
            ...
            if len(convert_list) > 0:
                ...
                convert_tasks = [self._convert_raw_files(old_dir, new_dir) for old_dir, new_dir in convert_list]  # noqa: E501
    ...
    def _read_image_dir(self) -> dict:
        ...
        with PerformanceTimer(timer_name="ReadingImageDirectory", logger=self._logger):
            ...
            for metadata in metadata_list:
                ...
                if list_type:
                    metadata[ExifTag.CREATE_DATE.value] = metadata.get(
                        ExifTag.CREATE_DATE.value, self.EXIF_UNKNOWN
                    ).replace(':','').replace(' ','_')
            ...
            # TODO: if there is no date and time: get the date from the fir name and set time to 'xxxxxx'
